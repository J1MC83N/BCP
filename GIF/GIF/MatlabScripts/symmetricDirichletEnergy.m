function [foldoversIndices, fz,fbz,E,G,H ] = symmetricDirichletEnergy(x,J_fz,J_fbz,Area,FixedIndices,FixedValues,SPDHessian, i)
...
if nargin<6
    ...
    sigma=abs_fz-abs_fbz;
    foldoversIndices = find(sigma < 0);
    if(i == 0)
        J_fz(foldoversIndices, :) = [];
        J_fbz(foldoversIndices, :) = [];
        fz(foldoversIndices, :) = [];
        fbz(foldoversIndices, :) = [];
        SIGMA(foldoversIndices, :) = [];
        sigma(foldoversIndices, :) = [];
        Area(foldoversIndices, :) = [];
        abs_fz(foldoversIndices, :) = [];
        abs_fbz(foldoversIndices, :) = [];
    end
    ...
    function [G_complex, grad_h_SIGMA, grad_h_sigma, grad_SIGMA_x, grad_sigma_x, grad_abs_fz, grad_abs_fbz] = calcGradient
        ...
        fbz_direction = fbz./abs_fbz;
        if any(abs_fbz==0)
            fbz_direction(abs_fbz==0)=1;
        end
        grad_abs_fbz=bsxfun(@times,conj(J_fbz),fbz_direction);% B'*B*x/norm(B*x) (where B*x=fbz) for each triangle
