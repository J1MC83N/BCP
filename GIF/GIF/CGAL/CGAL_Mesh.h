...
public:
	...
	void updateHalfedgesGlobalIndices();
	void updateVerticesGlobalIndices();
	void updateFacesGlobalIndices();
...
template<class kernel, class items>
void Polyhedron<kernel, items>::updateHalfedgesGlobalIndices()
{
	int numHalfEdges = size_of_halfedges();
	assert(numHalfEdges > 0);
	mHalfEdgeIndexMap.resize(numHalfEdges);
...
template<class kernel, class items>
void Polyhedron<kernel, items>::updateVerticesGlobalIndices()
{
	int numVertices = size_of_vertices();
	assert(numVertices > 0);
...
template<class kernel, class items>
void Polyhedron<kernel, items>::updateFacesGlobalIndices()
{
	int numFaces = size_of_facets();
	assert(numFaces > 0);
