...
double HalfedgeBase<Refs, Traits>::meanValueWeight(bool useTargetMetric = false) const
{
	...
	if(useTargetMetric) //compute the mean value weights based on the assigned edge lengths
	{
		...
		//When using meanvalue weights taken from a target metric, it is possible to have different lengths for the two halfedges.
		//This can happen when we glue two meshes together where each one has its own metric. 
		//Trying to make the metric consistent by using the same length on both sides (or averaging the lengths) can lead to triangle inequality violation.
		//On each triangle (there are two triangles influencing the edge weight), the metric is valid.
		//Hence, we use each triangle own lengths to calculate the corresponding angles.		double f = opposite()->mTargetMetric;
		double f = opposite()->mTargetMetric;
		...
		double nominatorB = (f-d-e)*(f+d-e);
		double denominatorB = (d+e+f)*(d-e-f);
		...
		if((e+f > d) && (d+f > e) && (d+e > f)) //check triangle inequality
		{
		...
		//double weight = (alphaA_half + alphaB_half)/f;
		//double weight = 2.0*(alphaA_half + alphaB_half)/(c+f);
...
double HalfedgeBase<Refs, Traits>::cot(bool useTargetMetric) const
{
	...
	if(useTargetMetric) //compute the cot weights based on the assigned edge lengths
	{
		...
		{
			cerr << "HalfedgeBase::cot(), Error at face: " << face()->index() << ". Triangle inequality doesn't hold" << endl;
