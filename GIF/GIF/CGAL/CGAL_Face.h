...
double FaceBase<Refs, Traits>::doubleArea() const
{
	...
	if(!h->is_triangle()) //this face is not a triangle. here we assume that the polygon is simple and lies on a plane. otherwise the output area will be wrong
	{
		...
		// ZL I disabled this check. I don't get it: if I supersample an edge, e.g. SelfOverlapping::superSamplePolygonEdges(), then vertices would be collinear. Also, we calculate doublePolygonArea afterwards anyway, so what is this check for?
		if ( 0 ) {
			if ( !(doublePolygonArea > 1e-6) )
				Log.warn("FaceBase::doubleArea(), assertion doublePolygonArea=%g > 1e-6 failed", doublePolygonArea);
