...
// ZL https://stackoverflow.com/questions/2891766/how-to-throw-an-exception-in-c
#include <setjmp.h>
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (ptrdiff_t) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
    ...
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      printf("    [6] = x%lx  %d\n", (ptrdiff_t) printsh.ss,
             printsh.ssorient);
    }
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (ptrdiff_t) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (ptrdiff_t) printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (ptrdiff_t) (pool->nowblock + 1);
...

{
  ...
  alignptr = (ptrdiff_t) (pool->pathblock + 1);
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (ptrdiff_t) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (ptrdiff_t) m->triangles.alignbytes -
                         (alignptr %
                          (ptrdiff_t) m->triangles.alignbytes));
