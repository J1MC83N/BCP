...
#ifndef ISO_TLC_SEA_GEO_UTIL_H
#define ISO_TLC_SEA_GEO_UTIL_H
...
// return: angle in [0, 2pi)
double compute_vector_angle(const PointE &p);
...
double compute_total_signed_area(const std::vector<PointE> &vertices,
...
double compute_total_signed_area_with_gradient(const std::vector<PointE> &vertices,
                                               const std::vector<std::pair<size_t,size_t>> &edges,
...
double compute_tri_signed_area(const PointE& p1, const PointE& p2, const PointE& p3);
