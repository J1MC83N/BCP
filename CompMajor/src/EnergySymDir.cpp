...
double DistortionSymDir::evaluate_small_k(const Vec& X)
{
	UpdateSSVDFunction();

	// E = ||J||^2+||J^-1||^2 = ||J||^2+||J||^2/det(J)^2
	Eigen::VectorXd col0 = s.col(0);
	Eigen::VectorXd col1 = s.col(1);

	 //Compute (s(:, 0) - s(:, 1)) and (s(:, 0) + s(:, 1))
	Eigen::VectorXd diff = col0 - col1;
	Eigen::VectorXd sum = col0 + col1;
	Eigen::VectorXd k_i = diff.cwiseQuotient(sum);

	return (Area.asDiagonal()* k_i).sum();
}
